...
export default async function handler(req, res) {
  ...
  console.log(`${req.method} /api/getMealPlan/${req.query.userId}`);
  ...
  if (req.method === 'GET') {
    try {
      ...
      console.log(`Fetching meal plans for user ${userId} from ${startDate.toISOString().split('T')[0]} to ${endDate.toISOString().split('T')[0]}`);
      ...
      console.log(`Grouped into ${Object.keys(mealPlansByDay).length} days`);

      return res.status(200).json({
        success: true,
        mealPlansByDay,
        hasPlans: Object.keys(mealPlansByDay).length > 0,
        isExisting: Object.keys(mealPlansByDay).length > 0
      });
